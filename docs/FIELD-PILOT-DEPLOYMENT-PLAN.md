...
**Our own network does not mean an isolated one.** The router owns the LAN; its **WAN port is optional** and can be fed by an ethernet drop, by the site's Wi-Fi *joined as a client* (repeater/WISP mode), or by a cellular SIM. Tablets and server keep our subnet and our addresses in all three cases, so an uplink can be added, removed, or fail with **no clinical consequence** — while when present it buys **Android-native tablet clock sync** (§3.4). **Remote support no longer rides this uplink at all**: it rides the *laptop's own* connection to a third-party Wi-Fi (§3.5), which is what MSF proved on 2026-09-22 — and which is the only option that reaches an 802.1X corporate network, since GL.iNet repeater mode is PSK-only. Equipment consequence: a WAN port *and* a wireless client mode are requirements, not preferences.
...
The site is offline for clinical use, but internet is expected intermittently. Two capabilities ride on that occasional connectivity: a **remote-support channel** and a **data export**. The export is patient data leaving the kit and still requires MSF data-protection sign-off (§8). The support channel's governance changed on **2026-09-22** — see the status block.

> ### ✅ Status 2026-09-22: DECIDED — TeamViewer, MSF's own, proven in MSF's own environment
>
> **MSF installed TeamViewer on the server laptop themselves, joined that laptop to their corporate Wi-Fi while it stayed wired to our router, and confirmed both that the tablets kept working and that TeamViewer access worked.** That is the remote-support channel for the pilot. It is judged good enough *for the pilot*; whether it continues beyond it will be decided on experience.
>
> **What MSF's test actually proved**, worth recording separately from the tool choice, because it validates the topology rather than the product:
> 1. **Dual-homed operation works.** The server laptop holds our LAN on the wire and a third-party Wi-Fi at the same time, and the tablets were unaffected. That is the arrangement the kit now assumes, and it was proven in MSF's own environment rather than on our bench.
> 2. **Corporate Wi-Fi association works** — whatever authentication their network uses, the laptop handled it. A router-side uplink could not have been relied on for this: GL.iNet repeater mode is PSK-only, so an 802.1X corporate network is reachable by the *laptop* and not by the router. This is why the uplink sits on the laptop (§3.3).
> 3. **Nothing clinical depends on it.** Server and tablets keep our subnet and our addresses whether the uplink is present, absent, or failing.
>
> **How it is run is MSF's to decide, and the working assumption is attended access (2026-09-23).** When support is needed, **someone on site opens TeamViewer**. We do not require unattended access, and we do not require the machine to be left permanently logged in and reachable. MSF may of course choose to configure unattended access; that is their call, and B8 asks them to confirm or correct the assumption.
>
> **What this costs us, stated honestly.** TeamViewer is a remote *desktop*: a human drives a mouse. **Nothing is scriptable** — no automated diagnostic pull, no file sync, no unattended export. And with attended access, **we cannot look at a sick server unless somebody is on site, available and able to open TeamViewer** — so nothing is diagnosable out of hours, or at all while the site cannot be reached by phone. That is accepted rather than regretted, but it changes where the weight falls: it raises the value of the **offline USB diagnostic dump** (§3.4), which needs neither connectivity nor a supporter, and of the server **recovering by itself after a power cut** (§3.1 — `restart: unless-stopped`, Docker enabled at boot, addressing applied by the setup script), which is what stops an unreachable site becoming an outage.
>
> **A trade we considered and declined.** Requiring the box to be reachable after an unattended reboot would have meant configuring **desktop autologin** — an unattended, permanently logged-in desktop holding patient data, on a machine that non-technical staff power-cycle. Attended access means that trade is simply never taken. Recorded so it is not rediscovered later and quietly accepted as a convenience.
>
> **An assumption this overturns, recorded rather than deleted.** This section previously argued that "the server is headless, so no remote *desktop* is needed; SSH is the right tool." That was wrong on its premise — the server is a laptop with a screen, and MSF demonstrated the desktop route working in the real environment. The reasoning is kept because the plan's worth is the record of *why* decisions were made, including those that evidence overtakes.
>
> **Still worth having in writing:** MSF's confirmation that they consider remote support settled on their side. It is their tool, their account and their IT approval, so the decision is theirs to make and they have made it — but the pilot should not rely on a verbal account of it.

**The exposure this makes urgent, and it is not hypothetical.** While that laptop sat on MSF's corporate Wi-Fi, the stack published OpenMRS on `:9000` and the package server on `:9001` to `0.0.0.0` — reachable by anyone on that corporate network, serving patient data behind a single well-known account, alongside an APK that carries the server password as a readable string resource. The fix (binding those ports to the LAN address only) is in the package version now under test; on a dual-homed box it is the **main reason to ship that version**, not a refinement.

**The tunnel work is parked, not deleted (Tailscale + SSH).** It is built and documented, `ENABLE_REMOTE_SUPPORT=false` ships as the default, and `FIELD-PILOT-WS7-REMOTE-ACCESS-TEST-PLAN.md` plus its runsheet stay intact and runnable for whoever picks it up. It returns to the table if the pilot shows TeamViewer is not enough — specifically if support ever needs something **scriptable** (automated diagnostics, a file pull, an unattended export) or a session with **nobody at the keyboard** — which attended access, by design, does not provide. The original reasoning for it is preserved below.

**Parked rationale — the tunnel design (was: decided, Tailscale + SSH to start).** Skilled staff cannot be guaranteed on-site, so the kit includes an **opt-in secure tunnel**, **server-initiated** (dials out — no inbound ports, works behind the router's NAT with nothing configured at site), **dormant when offline**, connecting to a SolDevelo endpoint only when internet is present. Support is via **SSH** (terminal + a forwarded port to the OpenMRS web UI) — the server is headless, so **no remote *desktop* is needed**; "remote desktop, no VPN" is the mental model, SSH is the right tool. Transport starts with **Tailscale** (zero site-side config, NAT-traversing; it is technically a WireGuard mesh VPN, but nothing is configured at site — the honest framing to MSF is "the server dials out to us, no inbound access, no VPN client for you to manage"). Its coordination plane is a third-party service (cannot read the end-to-end-encrypted traffic, but in the path), so the **data-protection sign-off may push to self-hosted WireGuard/Headscale** — the plan is written around "server-initiated dial-out + SSH" so the transport can be swapped without rework. Properties: authenticated, encrypted, server-initiated, controllable, auditable, switch-off-able; **ships disabled** (`ENABLE_REMOTE_SUPPORT=false`), now because the pilot uses TeamViewer rather than because sign-off is pending. It is a **support side-channel only** — clinical operation never depends on it. When there is no connectivity, the offline `buendia-diagnostics` USB dump (§3.4) is the fallback.

**Data extraction.** So MSF medical teams are not "blind" to the data, the kit provides a basic export of the clinical data, leveraging the module's existing `DataExportServlet` (CSV). The export is written to a **USB key at the server**; with TeamViewer as the channel there is no unattended pull, so a remote session means a human saving the file and transferring it, not an automated fetch. **This half of C1 is unchanged** — the export moves patient data off the kit regardless of how it travels, so its format, destination and access remain governed by MSF's data-protection rules. As patient data leaving the kit, its format, destination, and access are governed by MSF's data-protection rules; a future option is to feed it into MSF's LIME EMR rather than flat files.
...
### WS-7 — Remote support channel + data export  *(§3.5; independent)*
- **Remote support (settled 2026-09-22): MSF's own TeamViewer on the server laptop**, over the laptop's own connection to a third-party Wi-Fi. MSF installed it and proved it in their environment. **How it is run is theirs to decide; the working assumption is attended access** — someone on site opens TeamViewer when support is needed (2026-09-23). Nothing to build; what remains is verification of the parts that are ours (the supporter can reach the OpenMRS web UI at the documented address, and the stack is indifferent to TeamViewer starting and stopping) plus MSF's confirmation in writing.
- **Safe dual-homing — the work this stream actually carries now.** Because the laptop joins a foreign network, the stack must not be published to it: published ports bound to the LAN address, the wired default route losing to any real uplink, and a status tool for diagnosis over a phone call. Delivered in the package version now under bench test.
- **Data export:** wire the existing `DataExportServlet` (CSV) into a one-command export that writes to a USB key. With TeamViewer there is no unattended pull — a remote session means a human saving and transferring the file.
- **Parked, not deleted:** the server-initiated tunnel (Tailscale + SSH), `ENABLE_REMOTE_SUPPORT=false`, with `FIELD-PILOT-WS7-REMOTE-ACCESS-TEST-PLAN.md` and its runsheet kept runnable. Picked up only if the pilot shows TeamViewer is not enough — i.e. if support needs something scriptable or a session with nobody at the keyboard.
- Gate: the **export** half still needs MSF data-protection sign-off (§8). The support-channel half is MSF's own tool on MSF's own account, so that objection largely dissolves.

**Acceptance:** a supporter reaches the server over TeamViewer and completes a diagnosis; the box survives a reboot and is reachable again with only the on-site person's help; the export writes to USB; and with no connectivity at all, clinical operation is unaffected and the `buendia-diagnostics` dump remains the fallback.
...
4. **Operations & support tools:** the `buendia-diagnostics` dump script; the remote-support channel (MSF's TeamViewer) and the USB data export per §3.5 (the export subject to data-protection sign-off); the **router configuration in both forms — the `uci` script and the exported backup file** (WS-8).
...
| 6 | WS-5 APK delivery (QR first install + in-zone card); WS-7 remote support + data export; **WS-8 router config script (needs the router in hand)**; image tarball export; bundle assembly. |
...
The §3.4 offline-resilience items (server NTP + graceful shutdown, log caps, the `buendia-diagnostics` script, in-zone QR cards) are part of WS-1 (server/setup script) and WS-6 (tools/docs) — roughly +0.5–1 day, absorbed by the buffer. The **DNS-intercept half of the time sync and the router-config artefacts are now WS-8** (~0.5–1 day, needs the procured router). WS-7 is now much smaller than the ~1–2 days originally carried: the support channel is MSF's own TeamViewer and needs verification rather than construction, leaving the safe-dual-homing fixes (delivered) and the USB export, which is still gated on data-protection sign-off.
...
- **Remote access & data export expose patient data** — the support session reaches a box holding patient data, and the export takes clinical data off the kit (§3.5). **The sharper, demonstrated risk is the dual-homed box itself:** when the laptop joins a foreign Wi-Fi, anything the stack publishes to `0.0.0.0` is offered to that network — which already happened on MSF's corporate Wi-Fi during their TeamViewer test. Mitigations: published ports bound to the LAN address only, the export's format and destination governed by MSF data policy, and remote support being **MSF's own tool on MSF's own account**, which puts the access decision with the data owner. Clinical operation never depends on the channel; with no connectivity, support falls back to the `buendia-diagnostics` USB dump (§3.4).
...
- **Remote support (2026-09-22): MSF's own TeamViewer**, over the server laptop's own connection to a third-party Wi-Fi, with a person on site able to reboot and make sure it is up. Good enough for the pilot; continuation judged on experience. *(Supersedes: an opt-in server-initiated Tailscale + SSH tunnel, which is parked and kept runnable — and whose "the server is headless so no remote desktop is needed" premise MSF's test disproved.)* Data export stays `DataExportServlet` CSV to USB and **still needs data-protection sign-off** (§3.5).
...
**MSF decisions:**
5. **Data-protection sign-off** — now narrowed to the **data export** (§3.5). The remote-support half is settled in practice: MSF chose and installed their own TeamViewer, on their account, with their IT's approval. Still worth their written confirmation that they consider it closed.
...
7. ✅ **Closed — network topology.** Our own router; see *Decided* above and §3.3. What it leaves open is item 3 (the site layout, which sizes coverage) and the optional-uplink question in **B2** — now a smaller question than it was: remote support rides the *laptop's* connection (§3.5), so a router uplink buys tablet clock sync rather than access.
